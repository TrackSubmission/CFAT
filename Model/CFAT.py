...
###############------Channel-Wise_Attention_Block(CWAB)------###############
...
class CWAB(nn.Module):
    ...
    def __init__(self, num_feat, compress_ratio=3, squeeze_factor=30):
        super(CWAB, self).__init__()

        self.cwab = nn.Sequential(nn.Conv2d(num_feat, num_feat // compress_ratio, 3, 1, 1),
                                 nn.GELU(),
...
class WindowAttention_D(nn.Module):
    r""" Dense Window based multi-head self attention (DW-MSA) module with relative position bias.
...
class WindowAttention_S(nn.Module):
    r""" Sparse Window based multi-head self attention (SW-MSA) module with relative position bias.
    It supports both of shifted and non-shifted window.
    ...
    def forward(self, x, rpi, mask=None, sp_mask=None):  #[16, 16*16, 180]
        ...
        if sp_mask is not None:
            nP = sp_mask.shape[0]
            attn = attn.view(b_//nP, nP, self.num_heads, n, n) + sp_mask.unsqueeze(1).unsqueeze(0)  # (B, nP, nHead, N, N)
            attn = attn.view(-1, self.num_heads, n, n)
            if mask is not None:
                nw = mask.shape[0]
                attn = attn.view(b_// nw, nw, self.num_heads, n, n) + mask.unsqueeze(1).unsqueeze(0)   #[32, 6, 256, 256] [16, 256, 256]
...
###################---------Hybrid_Attention_Block_Dense(Dense_Att)---------###################
class Dense_Att(nn.Module):
    r""" Dense Attention Unit.
    ...
    def __init__(self,
        ...
        self.conv_block = CWAB(num_feat=dim, compress_ratio=compress_ratio, squeeze_factor=squeeze_factor)
    ...
    def forward(self, x, x_size, rpi_wab, attn_mask, triangular_masks):  #[1, 4096, 180], (64, 64)
        h, w = x_size
        b, _, c = x.shape
        ...
        conv_x = conv_x.permute(0, 2, 3, 1).contiguous().view(b, h * w, c)   #[1, 4096, 180]
        ####----CWAB----####
        ...
        if not self.triangular_flag:
            x_windows = window_partition(shifted_x, self.window_size)                # nw*b, window_size, window_size, c #[1, 64, 64, 180]->[16, 16, 16, 180]
            x_windows = x_windows.view(-1, self.window_size * self.window_size, c)   # nw*b, window_size*window_size, c  #[16, 16*16, 180]
        if self.triangular_flag:
            x_windows  = window_partition_triangular(shifted_x, 2*self.window_size, triangular_masks)  # nw*b, window_size, window_size, c #[1, 64, 64, 180]->[16, 16, 16, 180]
            _, m, n, _ = x_windows.shape           #[b*(h_ws)*(w_ws), m, n, c]
            x_windows  = x_windows.view(-1, n, c)  #[b*(h_ws)*(w_ws)*m, n, c]  #[16, 16*16, 180]
        #######----Partition_Windows----########
        
        #######----W-MSA/SW-MSA----########
        attn_windows = self.attn(x_windows, rpi=rpi_wab, mask=attn_mask)  #[16, 256, 180]
        #######----W-MSA/SW-MSA----########
        
        #######----Merge_Windows----########
        if self.triangular_flag:
            attn_windows = attn_windows.view(-1, m, n, c)   #[b*(h_ws)*(w_ws), m, n, c]
            shifted_x  = window_reverse_triangular(attn_windows, 2*self.window_size, triangular_masks)  # nw*b, window_size, window_size, c #[1, 64, 64, 180]->[16, 16, 16, 180]
            shifted_x = shifted_x.view(b, h // (2*self.window_size), w // (2*self.window_size), c, 2*self.window_size, 2*self.window_size)
            shifted_x = shifted_x.permute(0, 1, 4, 2, 5, 3).contiguous().view(b, h, w, c)   #[1, 64, 64, 180]
        if not self.triangular_flag:
            attn_windows = attn_windows.view(-1, self.window_size, self.window_size, c)  #[16, 16, 16, 180]
            shifted_x = window_reverse(attn_windows, self.window_size, h, w)  # b h' w' c  #[16, 16, 16, 180]->[1, 64, 64, 180]
        #######----Merge_Windows----########
...
###################---------Hybrid_Attention_Block_Sparse(Sparse_Att)---------###################
class Sparse_Att(nn.Module):
    r""" Sparse Attention Unit.
    ...
    def forward(self, x, x_size, rpi_wab, attn_mask, triangular_masks):  #[1, 4096, 180], (64, 64)
        ...
        x = F.pad(x, (0, 0, pad_l, pad_r, pad_t, pad_b))  #[1, 64, 64, 180]-->[1, pad_t+64+pad_b, pad_l+64+pad_r, 0+180+0]
        ####----padding----####
        ...
        if not self.triangular_flag:
            ...
            x_windows = window_partition(shifted_sparse_x, self.window_size)                # nw*b, window_size, window_size, c #[1, 64, 64, 180]->[16, 16, 16, 180]
            x_windows = x_windows.view(-1, self.window_size * self.window_size, c)   # nw*b, window_size*window_size, c  #[16, 16*16, 180]
        if self.triangular_flag:
            #_, h_s, w_s, _ = shifted_sparse_x.shape  #Here: h_s=Gh and w_s=Gw
            assert Gh >= (2*self.window_size) and Gw >= (2*self.window_size), "input feature has wrong size" 
            x_windows  = window_partition_triangular(shifted_sparse_x, 2*self.window_size, triangular_masks)  # nw*b, window_size, window_size, c #[1, 64, 64, 180]->[16, 16, 16, 180]
            _, m, n, _ = x_windows.shape           #[b*(h_ws)*(w_ws), m, n, c]
            x_windows  = x_windows.view(-1, n, c)  #[b*(h_ws)*(w_ws)*m, n, c]  #[16, 16*16, 180]
        #######----Partition_Windows----########
        
        #######----W-MSA/SW-MSA----########
        attn_windows = self.attn(x_windows, rpi=rpi_wab, mask=attn_mask, sp_mask=attn_mask_sp)  #[16, 256, 180]
        #######----W-MSA/SW-MSA----########
        
        #######----Merge_Windows----########
        if self.triangular_flag:
            attn_windows = attn_windows.view(-1, m, n, c)   #[b*(h_ws)*(w_ws), m, n, c]
            shifted_sparse_x  = window_reverse_triangular(attn_windows, 2*self.window_size, triangular_masks)  # nw*b, window_size, window_size, c #[1, 64, 64, 180]->[16, 16, 16, 180]
            shifted_sparse_x = shifted_sparse_x.view(-1, Gh // (2*self.window_size), Gw // (2*self.window_size), c, 2*self.window_size, 2*self.window_size)
            shifted_sparse_x = shifted_sparse_x.permute(0, 1, 4, 2, 5, 3).contiguous().view(-1, Gh, Gw, c)   #[1, 64, 64, 180]
        if not self.triangular_flag:
            attn_windows = attn_windows.view(-1, self.window_size, self.window_size, c)  #[16, 16, 16, 180]
            shifted_sparse_x = window_reverse(attn_windows, self.window_size, Gh, Gw)  # b h' w' c  #[16, 16, 16, 180]->[1, 64, 64, 180]
        #######----Merge_Windows----########

        #######----Reverse_Sparse----########
        shifted_sparse_x = shifted_sparse_x.reshape(b, I, I, Gh, Gw, c).permute(0, 3, 1, 4, 2, 5).contiguous()  # b, Gh, I, Gw, I, c
...
###################---------Overlapping_Cross_Fusion-Attention_Block(OCFAB)---------###################
class OCFAB(nn.Module):
    def __init__(self, dim,
        ...
        self.relative_position_bias_table = nn.Parameter(torch.zeros((window_size + self.overlap_win_size - 1) * (window_size + self.overlap_win_size - 1), num_heads))  
                                                                                            # 2*Wh-1 * 2*Ww-1, nH
        trunc_normal_(self.relative_position_bias_table, std=.02)
        self.softmax = nn.Softmax(dim=-1)
    ...
    def forward(self, x, x_size, rpi):
        ...
        ##########------relative_position_bias------##########
        relative_position_bias = self.relative_position_bias_table[rpi.view(-1)].view(self.window_size * self.window_size, self.overlap_win_size * self.overlap_win_size, -1)  
                                                                                        # ws*ws, wse*wse, nH
...
###################---------Atten_Blocks(Dense_Att+Sparse_Att+OCFAB)---------###################
class AttenBlocks(nn.Module):
    """ A series of attention blocks in one HAP.
    ...
    def __init__(self,
        ...
        ##Dense_Att+Sparse_Att
        if not sparse_flag:
            self.blocks = nn.ModuleList([Dense_Att(dim=dim,
            ...
            self.blocks = nn.ModuleList([Sparse_Att(dim=dim,
                                            input_resolution=input_resolution,
                                            num_heads=num_heads,
                                            window_size=window_size,
                                            shift_size=shift_size[i],
        ...
        ##OCFAB
        self.overlap_attn = OCFAB(dim=dim,
                                 input_resolution=input_resolution,
                                 window_size=window_size,
    ...
    def forward(self, x, x_size, params):  #[1, 4096, 180], (64, 64)
        for blk in self.blocks:
            x = blk(x, x_size, params['rpi_wab'], params['attn_mask'], params['triangular_masks'])

        x = self.overlap_attn(x, x_size, params['rpi_ocfab'])  #[1, 4096, 180]->[1, 4096, 180]
...
###################---------Hybrid_Attention_Pool(HAP)---------###################
class HAP(nn.Module):
    """Hybrid Attention Pool (HAP).
...
class cfat(nn.Module):
    ...
    def __init__(self,
        ...
        relative_position_index_WAB = self.calculate_rpi_WAB()
        relative_position_index_OCFAB = self.calculate_rpi_OCFAB()
        self.register_buffer('relative_position_index_WAB', relative_position_index_WAB)
        self.register_buffer('relative_position_index_OCFAB', relative_position_index_OCFAB)
        ######----Relative_Position_Index----######
    ...
    ######----Relative_Position_Index_for_WAB----######
    def calculate_rpi_WAB(self):
    ...
    ######----Relative_Position_Index_for_OCFAB----######
    def calculate_rpi_OCFAB(self):
    ...
    def forward_features(self, x):  #[1, 180, 64, 64]
        ...
        attn_mask = tuple([self.calculate_mask(x_size, shift_size).to(x.device) for shift_size in (8, 16, 24)])  #[16, 256, 256]   #changed to tuple
        triangular_masks = tuple(self.triangle_masks(x))  #[16, 256, 256]   #changed to tuple
        
        params = {'attn_mask': attn_mask, 'triangular_masks': triangular_masks, 'rpi_wab': self.relative_position_index_WAB, 'rpi_ocfab': self.relative_position_index_OCFAB}
